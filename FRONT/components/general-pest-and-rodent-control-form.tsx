...
interface GeneralPestAndRodentControlFormProps {
...
export default function GeneralPestAndRodentControlForm({ client, session, onFormSubmit }: GeneralPestAndRodentControlFormProps) {
  ...
  const [summary, setSummary] = useState("Ditemukan jejak tikus di area dapur dan aktivitas serangga di area lembab.");
  const [recommendation, setRecommendation] = useState("Pemasangan bait station di perimeter luar, perangkap di dalam, dan aplikasi residual spraying untuk serangga.");
  ...
  const uploadImage = async (file: File) => {
    ...
    try {
      // NOTE: This uses a generic endpoint. Ensure it exists on your backend.
  ...
  const handleSubmit = () => {
    ...
    const finalResult: UnifiedResultData = {
      serviceType: 'GPRC', // General Pest and Rodent Control
      client: client,
      ...
      inspection: {
        ...
        treatment: "Integrated Pest Management (GPRC)",
      ...
      details: {
        ...
        // Other GPRC specific fields can be added here if needed
        tingkatInfestasi: 'Sedang', // Example value
        targetHama: ['Tikus', 'Kecoa', 'Semut'], // Example value
      }
    };
  ...
  return (
    <>
      {isClient && isCameraOpen && createPortal(
        <div className="fixed inset-0 bg-black/90 z-50 flex flex-col items-center justify-center p-4">
          ...
          <div className="flex items-center gap-4 mt-4">
            <Button onClick={handleCapture} className="bg-purple-500 hover:bg-purple-600 text-black font-bold rounded-full p-4 h-16 w-16">
      ...
      <Card className="p-6 bg-black/90 border-l-4 border-purple-500 text-white space-y-8">
        {/* --- FORM HEADER --- */}
        <div className="flex items-center gap-2">
          <Bug className="h-6 w-6 text-purple-500" />
          <h2 className="text-xl font-bold headline">Formulir General Pest & Rodent Control (GPRC)</h2>
        </div>
        ...
        <div className="space-y-6">
          <div>
            ...
            <Input
              ...
              className="mt-2 w-full bg-black/50 border-purple-600 text-white"
              placeholder="Contoh: Jl. Gatot Subroto No. 12, Bandung"
            />
          </div>
          <div>
            ...
            <div className="flex items-center gap-4 mt-2">
              ...
              <Input type="number" value={luasTanah} onChange={(e) => setLuasTanah(Number(e.target.value))} className="w-24 bg-black/50 border-purple-600" />
            </div>
          </div>
        </div>

        <div className="border-t-2 border-dashed border-purple-800/50 my-8"></div>
          ...
          <div className="grid md:grid-cols-2 gap-6">
            <div>
              ...
              <Select value={transport} onValueChange={(value) => setTransport(value as 'mobil' | 'motor')}>
                <SelectTrigger id="transport" className="mt-2 bg-black/50 border-purple-600 text-white">
                  <SelectValue />
                </SelectTrigger>
            ...
            <div>
              ...
              <Input type="number" id="jarak-tempuh" value={jarakTempuh} onChange={(e) => setJarakTempuh(Number(e.target.value))} className="mt-2 w-full bg-black/50 border-purple-600" />
            </div>
            <div>
              <Label htmlFor="jumlah-lantai">Jumlah Lantai</Label>
              <Input type="number" id="jumlah-lantai" value={jumlahLantai} onChange={(e) => setJumlahLantai(Number(e.target.value))} className="mt-2 w-full bg-black/50 border-purple-600" />
            </div>
            <div>
              <Label htmlFor="monitoring-per-bulan">Durasi Monitoring (Bulan)</Label>
              <Input type="number" id="monitoring-per-bulan" value={monitoringPerBulan} onChange={(e) => setMonitoringPerBulan(Number(e.target.value))} className="mt-2 w-full bg-black/50 border-purple-600" />
            </div>
          </div>
        </div>
        ...
        <div>
          ...
          <div className="space-y-6">
            ...
            <div>
              ...
              <div className="mt-2 p-4 border-2 border-dashed border-purple-800/50 rounded-lg">
                ...
                <div className="grid sm:grid-cols-2 gap-4">
                  <Button onClick={() => fileInputRef.current?.click()} disabled={isUploading} className="w-full bg-purple-600 hover:bg-purple-700 text-white font-bold">
            ...
            <div>
              ...
              <Textarea id="summary" value={summary} onChange={e => setSummary(e.target.value)} className="mt-2 bg-black/50 border-purple-600" rows={3} />
            </div>
            <div>
              <Label htmlFor="recommendation">Rekomendasi Penanganan</Label>
              <Textarea id="recommendation" value={recommendation} onChange={e => setRecommendation(e.target.value)} className="mt-2 bg-black/50 border-purple-600" rows={3} />
            </div>
          </div>
        </div>

        <Button onClick={handleSubmit} className="w-full bg-purple-500 hover:bg-purple-600 text-white font-bold text-lg py-6">
          Lanjutkan
        </Button>
      </Card>
    </>
