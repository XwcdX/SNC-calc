...
import { Rat, Calculator, Camera, Upload, Trash2, XCircle } from "lucide-react"; // Using Rat icon
import Image from "next/image";
import { createPortal } from "react-dom";

// Interface for inspection images
...
export default function RatControlForm({ client, session, onFormSubmit }: RatControlFormProps) {
  ...
  const [summary, setSummary] = useState("Ditemukan jejak tikus di area dapur dan plafon, seperti kotoran dan bekas gigitan.");
  const [recommendation, setRecommendation] = useState("Pemasangan bait station di perimeter luar dan perangkap di jalur-jalur utama tikus di dalam.");
  ...
  // Effect to handle body scroll when camera modal is open
  useEffect(() => {
  ...
  const handleSubmit = () => {
    ...
    const finalResult: UnifiedResultData = {
      ...
      details: {
        ...
        rekomendasiSanitasi: "Tutup semua sumber makanan dan perbaiki lubang di dinding untuk mencegah akses masuk kembali."
      }
    };
  ...
  return (
    <>
      {isClient && isCameraOpen && createPortal(
        <div className="fixed inset-0 bg-black/90 z-50 flex flex-col items-center justify-center p-4">
          ...
          <div className="flex items-center gap-4 mt-4">
            <Button onClick={handleCapture} className="bg-blue-500 hover:bg-blue-600 text-white font-bold rounded-full p-4 h-16 w-16">
      ...
      <Card className="p-6 bg-black/90 border-l-4 border-blue-500 text-white space-y-8">
        {/* --- FORM HEADER --- */}
        ...
        <div className="space-y-6">
           <div>
            ...
            <Input
              ...
              className="mt-2 w-full bg-black/50 border-blue-600 text-white"
              placeholder="Contoh: Jl. Sudirman No. 5, Jakarta"
            />
          </div>
          <div>
            ...
            <div className="flex items-center gap-4 mt-2">
              ...
              <Input type="number" value={luasTanah} onChange={(e) => setLuasTanah(Number(e.target.value))} className="w-24 bg-black/50 border-blue-600" />
            </div>
          </div>
        </div>

        <div className="border-t-2 border-dashed border-blue-800/50 my-8"></div>

        {/* --- OPERATIONAL DETAILS SECTION (NEW) --- */}
        <div>
          ...
          <div className="grid md:grid-cols-2 gap-6">
             <div>
              ...
              <Select value={transport} onValueChange={(value) => setTransport(value as 'mobil' | 'motor')}>
                <SelectTrigger id="transport" className="mt-2 bg-black/50 border-blue-600 text-white"><SelectValue /></SelectTrigger>
            ...
            <div>
              ...
              <Input type="number" id="jarak-tempuh" value={jarakTempuh} onChange={(e) => setJarakTempuh(Number(e.target.value))} className="mt-2 w-full bg-black/50 border-blue-600" />
            </div>
             <div>
              <Label htmlFor="jumlah-lantai">Jumlah Lantai</Label>
              <Input type="number" id="jumlah-lantai" value={jumlahLantai} onChange={(e) => setJumlahLantai(Number(e.target.value))} className="mt-2 w-full bg-black/50 border-blue-600" />
            </div>
            <div>
              <Label htmlFor="monitoring-per-bulan">Durasi Monitoring (Bulan)</Label>
              <Input type="number" id="monitoring-per-bulan" value={monitoringPerBulan} onChange={(e) => setMonitoringPerBulan(Number(e.target.value))} className="mt-2 w-full bg-black/50 border-blue-600" />
            </div>
          </div>
        </div>
        ...
        {/* --- INSPECTION DATA SECTION (UPDATED) --- */}
        <div>
          <div className="flex items-center gap-2 mb-6">
          ...
          <div className="space-y-6">
            ...
            <div className="grid md:grid-cols-3 gap-6">
              <div>
                ...
                <Select value={tingkatInfestasi} onValueChange={(v) => setTingkatInfestasi(v as 'Rendah' | 'Sedang' | 'Tinggi')}>
              ...
              <div>
                ...
                <Input type="number" value={jumlahBaitStation} onChange={e => setJumlahBaitStation(Number(e.target.value))} className="mt-2 bg-black/50 border-blue-600" />
              </div>
              <div>
                <Label>Jumlah Perangkap</Label>
                <Input type="number" value={jumlahPerangkap} onChange={e => setJumlahPerangkap(Number(e.target.value))} className="mt-2 bg-black/50 border-blue-600" />
              </div>
            </div>
            ...
            <div>
              ...
              <div className="mt-2 p-4 border-2 border-dashed border-blue-800/50 rounded-lg">
                ...
                <div className="grid sm:grid-cols-2 gap-4">
                  <Button onClick={() => fileInputRef.current?.click()} disabled={isUploading} className="w-full bg-blue-600 hover:bg-blue-700 text-white font-bold">
                    <Upload className="h-4 w-4 mr-2" />
                    {isUploading ? "Mengunggah..." : "Unggah Foto"}
                  </Button>
                  <Button onClick={openCamera} disabled={isUploading} className="w-full bg-gray-600 hover:bg-gray-700 text-white font-bold">
            ...
            <div>
              ...
              <Textarea id="summary" value={summary} onChange={e => setSummary(e.target.value)} className="mt-2 bg-black/50 border-blue-600" rows={3}/>
            </div>
            <div>
              <Label htmlFor="recommendation">Rekomendasi Penanganan</Label>
              <Textarea id="recommendation" value={recommendation} onChange={e => setRecommendation(e.target.value)} className="mt-2 bg-black/50 border-blue-600" rows={3}/>
            </div>
          </div>
        </div>

        <Button onClick={handleSubmit} className="w-full bg-blue-500 hover:bg-blue-600 text-white font-bold text-lg py-6">
          Lanjutkan
        </Button>
      </Card>
    </>
